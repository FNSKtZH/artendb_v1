...
module.exports = function (level, filter, gruppe, id) {
  ...
  if (gruppe === 'lr') {
    url = $(window.location).attr('protocol') + '//' + $(window.location).attr('host') + '/artendb/_design/artendb/_list/baumLr/baumLr?startkey=[' + level + ', "' + id + '"]&endkey=[' + level + ', "' + id + '",{},{},{},{}]&group_level=6'
  } else {
    gruppe = capitaliseFirstLetter(gruppe)
    url = $(window.location).attr('protocol') + '//' + $(window.location).attr('host') + '/artendb/_design/artendb/_list/baum' + gruppe + '/baum' + gruppe + '?startkey=' + JSON.stringify(startkey) + '&endkey=' + JSON.stringify(endkey) + '&group_level=' + level
  }
  if (id2) {
    url = url + '&id=true'
  }
