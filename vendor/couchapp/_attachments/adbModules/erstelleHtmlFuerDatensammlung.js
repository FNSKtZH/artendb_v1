...
var returnFunction = function (dsTyp, art, datensammlung) {
    ...
    html_datensammlung += '<h4 class="panel-title"><a class="Datensammlung accordion-toggle" data-toggle="collapse" data-parent="#panel_art" href="#collapse' + dsName + '">';
    // Titel für die Datensammlung einfügen
    html_datensammlung += datensammlung.Name;
    // header abschliessen
    html_datensammlung += '</a></h4></div>';
    // body beginnen
    html_datensammlung += '<div id="collapse' + dsName + '" class="panel-collapse collapse ' + art.Gruppe + ' ' + dsTyp + '"><div class="panel-body">';
    ...
    if (dsTyp === "Taxonomie") {
        html_datensammlung += erstelleHtmlFuerFeld("GUID", art._id, dsTyp, "Taxonomie");
    }
    _.each(datensammlung.Eigenschaften, function (feldwert, feldname) {
        if (feldname === "GUID") {
            ...
            html_datensammlung += window.adb.generiereHtmlFürTextarea(feldname, hierarchie_string, dsTyp, datensammlung.Name.replace(/"/g, "'"));
        } else if (_.isArray(feldwert)) {
            // dieses Feld enthält einen Array von Werten
            array_string = feldwert.toString();
            html_datensammlung += window.adb.generiereHtmlFürTextarea(feldname, array_string, dsTyp, datensammlung.Name.replace(/"/g, "'"));
        } else {
            html_datensammlung += erstelleHtmlFuerFeld (feldname, feldwert, dsTyp, datensammlung.Name.replace(/"/g, "'"));
        }
    });
